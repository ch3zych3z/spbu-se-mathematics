...
\begin{Thm}[Барроу, об интеграле с переменным верхним пределом] 
	$E \subset \R$ -- невыраженный промежуток, $f: E \to \R$, интегрируема на каждом отрезке из E, $a \in E$,
...
\begin{proof}
	\begin{enumerate}
		...
		|\Phi(x_0 + \Delta x) - \Phi(x_0)| = |\int_{a}^{x_0 + \Delta x} f - \int_{a}^{x_0} f| = 
		|\int_{x_0}^{x_0 + \Delta x} f| \leqslant |\int_{x_0}^{x_0+\Delta x} |f|| \leqslant 
		|\Delta x| \cdot M \underset{\Delta x \to 0}{\to} 0$
		\item Проверим, что $ \displaystyle \frac{\Phi(x_0 + \Delta x) - \Phi(x_0)}{\Delta x} \underset{\Delta x \to 0}{\to} f(x_0)$\\ %!
		Возьмем $\varepsilon > 0$ и $\delta > 0: \forall t: |t-x_0| < \delta  |f(t) - f(x_0)| < \varepsilon$ (по
		непрерывности.)\\ $\displaystyle \Big|\frac{\Phi(x_0 + \Delta x) - \Phi(x_0)}{\Delta x} - f(x_0) \Big| = \Big|\frac{1}{\Delta x} \int_{x_0}^{x_0 + \Delta x} 
		f(t)dt = f(x_0)\Big| = \Big| \frac{1}{\Delta x} \int_{x_0}^{x_0+\Delta x} (f(t) - f(x_0))dt \Big| < \\ < \frac{1}{|\Delta x|}
...
\begin{Example}
	$\displaystyle \int_{0}^{1} x^2 dx = \frac{x^3}{3} \Big|^1_0 = \frac{1}{3}$
\end{Example}

\begin{Example}
	$\displaystyle \int_{-1}^{1} \frac{1}{x^2} dx = - \frac{1}{x} \Big|^1_{-1} = -2$ - чушь!
	\begin{enumerate}
		\item $(-\frac{1}{x})' = \frac{1}{x^2}$ -- не везде на $[-1,1]$
...
\begin{Rem}
	$\displaystyle \int_{a}^{b} f dg = fg \Big|_a^b - \int_{a}^{b} gdf$\\
	$dg(x) = g'(x)dx$
\end{Rem}
...
\begin{proof}
	...
	$\displaystyle \sum_{k=0}^{n-1} \frac{f^{(k)}(a)}{k!} (x-a)^k + \frac{1}{(n-1)!} \int_{a}^{x} f^{(n)}(t) (x-t)^{n-1}dt = \sum_{k=0}^{n-1} \frac{f^{(k)}(a)}{k!}(x-a)^k +
	\frac{1}{(n-1)!} (-f^{(n)}(t) \cdot \frac{(x-t)^n}{n} \Big|^x_{t=a} + 
	\int_{a}^{x} \frac{f^{n+1}(t)(x-t)^n}{n}dt) = \sum_{k=0}^{n} \frac{f^{(k)}(a)}{k!} (x-a)^k + \frac{1}{n!} \int_{a}^{x} f^{(n+1)}(t) (x-t)^n dt$
\end{proof}
...
\begin{Thm}[Вторая теорема о среднем для интегралова, Бонне] 

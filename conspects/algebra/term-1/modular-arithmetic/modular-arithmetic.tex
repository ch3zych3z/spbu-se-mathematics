...
\begin{proof}
    \begin{MyList}
        ...
        $a_1x \equiv b_1 \Mod m$ -- $\exists$ решение $x_0 : a_1x_0 \equiv b_1 \Mod m_1$. $x = x_0 + m_1 t$ -- решение $ax \equiv b \Mod m$
...
\begin{Thm}[Методы решения $ax \equiv b \Mod m, (a, m) = 1$]
    \begin{MyList}
        ...
        \item $ax \equiv b \Mod m \SO x \equiv (-1)^m p_{m - 1} \cdot b \Mod m$ \\
        $\frac{m}{a}$ -- непрерывная дробь, $p_{m - 1}$ -- числитель $(m - 1)$-й подходящей дроби, $ \frac{p_n}{q_n} = \frac{m}{a}$  
    \end{MyList}
\end{Thm}

...
\textbf{Вывод}: можно спокойно преобразовывать АСК, степень многочлена при этом сохраняется.
 ...
 Хотим установить, существует ли у него центр симметрии. Для этого перенёесём начало координат, т.е. сделать перенос так, чтобы попасть на центр симметрии; если в него попасть, то исчезнет линейная часть. Мы знаем коэффициенты при $x'$ и $y'$, то есть они должны быть равны нулю, необходимо решить систему:
$\begin{cases}
...
\begin{MyList}
	\item $\delta \neq 0. \ \exists ! (x_0, x_0) $ -- центр симметрии. КВП -- \textbf{центральная}.
...
\begin{Def}
	...
	Другими словами: КВП называется центральной, если матричца главных членов невырожденная.
\end{Def}
...
\begin{Example}
	\begin{MyList}
		...
		\item Две прямые, паралелльные друг другу. У этой КВП будет центр симметрии, это любая точка на прямой между этими прямыми, которая прараллельна им и лежит на одинаковом расстоянии от них. Например, задать КВП как $y^2 = 1$, тогда центром будет $y = 0$.
	\end{MyList}
 \end{Example}
...
\end{array}\right|}, \ y_0 = \displaystyle \frac{-a_{23}a_{11} + a_{13}a_{12}}{\left| \begin{array}{cccc}
	a_{11} & a_{12}\\
	a_{12} & a_{22}
	\end{array}\right|}$, но и числитель можно интерпретировать как матрицу, т.е. можно записать решение системы в следущем виде:
...
\begin{Def}
	...
	$$a_{11}x'^2 + 2a_{12}x'y' + a_{22}y'^2 + \frac{\Delta}{\delta} = 0$$, где 

...
\begin{MyList}
    ...
    \begin{proof}
        $D(X) = \sum_{i=1}^{n} (X_i - E(X))^2 p_i = \sum_{i=1}^{n} X_i^2 p_i - \sum_{i=1}^{n} 2 X_i E(X) p_i + \sum_{i=1}^{n} (E(X))^2 p_i = E(X^2) - 2E(X) - E(X) + (E(X))^2 \cdot 1 = E(X^2) 0 (E(X))^2$ 
    \end{proof}
...
\begin{Def}
    ...
    \[r(X, Y) = \frac{\cov (X, Y)}{\sqrt{D(X) \cdot D(Y)}}\]
\end{Def}

Свойства:

\begin{MyList}
    \item $r(aX + b, cY + d) = \sign (ac) r(X, Y)$ 
\end{MyList}
